...
public class CanCommsHandler
{
    ...
    public CanCommsHandler(final String device, final CanFilter[] filters) throws IOException
    {
        ...
        rxNativeTask = () -> {
            ...
            nativeReceiveTask(receiverQueue, rxNativeTaskRunning, rxListenerTaskRunning, deviceFd);
        };
    ...
    private native void nativeReceiveTask(final LinkedBlockingQueue<CanMessage> receiveQueue, final AtomicBoolean running, final AtomicBoolean stopListener, final long deviceFd);
    ...
    {
        boolean success = (rxNativeThread.getState() == Thread.State.RUNNABLE) && (rxListenerThread.getState() == Thread.State.RUNNABLE);
        if (success)
        {
            try
            {
    ...
    public void addListener(final CanMessageListener canMessageListener)
    {
        canMessageListeners.add(canMessageListener);
    }

    public void removeListener(final CanMessageListener canMessageListener)
    {
